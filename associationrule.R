...
tdata3 <- as(itemlist, "transactions")  #transactions is a keyword
tdata3
summary(tdata3)
...
#freqitems = eclat(tdata, parameter = list(minlen=1, supp=.01 )) #try removing the comment and changing values if minlen and supp
...
#Construct the Rules
rules = apriori(tdata, parameter = list(supp = 0.01, conf = 0.1, minlen=2))
