...
colnames(data)
class(data$Age) 
apply(data, 2, FUN= class)  #are all numeric, 2 for clumns and 1 for rows
dim(data)                 #dimensions i.e 10867 rows and 13 columns
head(data)
summary(data)
...
k1<-amap::Kmeans(data[,-1],centers=3, iter.max = 200,nstart = 1, 
                 method = c("euclidean"))             #removing id column, iteration refines result
